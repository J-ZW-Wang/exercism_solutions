        ...
        while re.search(r"(\+-*\+)", line) != None:
            ...
            while top_side != None:  # Loop through all top sides in the 'line'
                ...
                while next_line_index < len(
                    strings
                ):  # loop through the verticle pattens
                    next_line = strings[next_line_index]
                    if (
                        next_line[s] in "|+" and next_line[e] in "|"
                    ):  # regular sidelines
                        next_line_index += 1
                    elif next_line[s] in "|" and next_line[e] in "|+":
                        next_line_index += 1
                    elif (
                        re.match(rf"\+[-\+]{{{ndashes}}}\+", next_line[s : e + 1])
                        != None
                    ):  # the lower side must be in the pattern of +-..--+
                        rec_counter += 1
                        print(
                            f"Boxes = {rec_counter}, topside = {top_side.group(0)}, line_index = {next_line_index} {next_line}"
                        )
                        next_line_index += 1
                    else:  # no box found
            ...
            line = line[:s] + "-" + line[s + 1 :]  # Replace the start of topside as '-'
